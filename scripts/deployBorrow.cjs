...
async function main() {
  ...
  const GreenFiBorrow = await ethers.getContractFactory("GreenFiBorrow");

  console.log("Deploying GreenFiBorrow...");

  // Deploy the contract as a UUPS proxy
  const greenFiBorrow = await upgrades.deployProxy(
    GreenFiBorrow,
    [weedlTokenAddress],
    {
  ...
  await greenFiBorrow.waitForDeployment();
  const greenFiBorrowAddress = await greenFiBorrow.getAddress();
  console.log("GreenFiBorrow proxy deployed to:", greenFiBorrowAddress);

  // Get and log the implementation address
  const implementationAddress = await upgrades.erc1967.getImplementationAddress(greenFiBorrowAddress);
  console.log("Implementation address:", implementationAddress);

  // Verify the WEEDL token address in the contract
  const deployedWeedlToken = await greenFiBorrow.weedlToken();
  ...
  try {
    await run("verify:verify", {
      address: greenFiBorrowAddress,
      constructorArguments: [], // Proxy has no constructor args
      contract: "contracts/GreenFiBorrow.sol:GreenFiBorrow",
    });
    console.log("Contract verified on Etherscan");
  } catch (error) {
    console.error("Etherscan verification failed:", error.message);
    console.log("You can manually verify the contract later using: npx hardhat verify --network sepolia", greenFiBorrowAddress);
  }
}
