...
import { GreenFiLand, GreenFiLoan, GreenFiBorrow, GreenFiSeed, GreenFiStaking } from "../config/contracts";
...
interface EnrichedBorrow extends Borrowing {
  metadata?: Metadata;
}
...
interface PlotUserData {
  ...
  seed?: EnrichedSeed;
  borrow?: EnrichedBorrow;
  loan?: EnrichedLoan;
  stake?: Staking;
  tooltip: string;
}

// Contract Event ABIs (unchanged)
...
export default function VirtualLandShowcase({ address }: { address: string }) {
  ...
  const [enrichedBorrows, setEnrichedBorrows] = useState<EnrichedBorrow[]>([]);
  ...
  const { data: landData, error: landError, refetch: refetchLandData } = useReadContract({
    ...
    query: {
      enabled: !!address,
      refetchInterval: isCorsError ? false : 10000, // Disable polling on CORS error, otherwise poll every 10s
      retry: 2, // Limit retries to prevent continuous requests
      retryDelay: 2000,
    },
  ...
  const { data: borrowData, error: borrowError } = useReadContract({
    address: GreenFiBorrow.address as `0x${string}`,
    abi: GreenFiBorrow.abi as Abi,
    functionName: "getAllBorrowings",
    query: {
      enabled: !!address,
      refetchInterval: isCorsError ? false : 10000,
      retry: 2,
  ...
  }) as { data: Borrowing[] | undefined; error: Error | null };
  ...
  // Event listeners for contract updates (optimized for fewer requests)
  useWatchContractEvent({
    ...
    poll: !isCorsError, // Disable polling on CORS error
  });
  ...
  useEffect(() => {
    const errors = [
      ...
      { error: borrowError, message: "Failed to fetch borrow listings", refetch: () => {} },
    ...
    errors.forEach(({ error, message, refetch }) => {
      if (error) {
        const errorMessage = error.message.includes("Pausable: paused")
          ? "Contract is paused."
          : error.message.includes("CORS")
          ? "CORS policy error: Blockchain provider does not allow requests from localhost. Consider using a proxy server or configuring provider CORS settings."
          : error.message;
  ...
  }, [landError, loanError, borrowError, seedError, stakeError, refetchLandData, refetchLoanData, refetchSeedData, refetchStakeData]);

  // Data loading with memoization (unchanged)
  const loadData = useMemo(
    () => ({
      ...
      async loadBorrows() {
        ...
        try {
          const enriched = await Promise.all(
            borrowData
              .filter((borrow: Borrowing) => borrow.isActive && borrow.borrower.toLowerCase() === address.toLowerCase())
              .map(async (borrow: Borrowing): Promise<EnrichedBorrow | null> => {
                const metadata = borrow.metadataURI ? await fetchMetadataWithRetry(borrow.metadataURI) : undefined;
                return { ...borrow, metadata };
              })
          );
          return enriched.filter((b): b is EnrichedBorrow => b !== null) as EnrichedBorrow[];
        } catch (error) {
          console.error("Borrow listings error:", error);
          toast.error("Error loading borrow listings.");
          return [];
        }
      },
    ...
    [landData, seedData, borrowData, loanData, stakeData, address]
  );
  ...
  useEffect(() => {
    Promise.all([
      ...
      loadData.loadBorrows().then((borrows) => setEnrichedBorrows(borrows as EnrichedBorrow[])),
      loadData.loadLoans().then((loans) => setEnrichedLoans(loans as EnrichedLoan[])),
      loadData.loadStakes(),
    ])
      .then(() => setIsLoading(false))
      .catch((error) => {
        setError("Failed to initialize 3D scene data.");
  ...
  useEffect(() => {
    ...
    const addPlots = async <T extends EnrichedListing | EnrichedSeed | EnrichedBorrow | EnrichedLoan | Staking>(
      items: T[],
      type: PlotType,
      getTooltip: (item: T, loanData?: Loan[], borrowData?: Borrowing[]) => string,
      ...
      for (const item of items) {
        ...
        instancedMesh.userData[instanceIndex] = {
          ...
          tooltip: getTooltip(item, loanData, borrowData),
        } as PlotUserData;
        instanceIndex++;
      }
    ...
    const addLandListings = () =>
      addPlots<EnrichedListing>(
        ...
        (listing) => {
          ...
          const borrowing = borrowData?.find((b: Borrowing) => b.id === listing.borrowId && b.isActive);
          return `
            ...
            ${borrowing ? `<strong>Borrowing:</strong> ${(Number(borrowing.fee) / 1e18).toFixed(4)} WEEDL, ${(Number(borrowing.duration) / 86400).toFixed(0)} days, Purpose: ${sanitizeHTML(borrowing.purpose)}<br>` : ""}
    ...
    const addBorrowListings = () =>
      addPlots<EnrichedBorrow>(
        enrichedBorrows,
        PLOT_TYPES.BORROW,
        (borrow) => `
          <strong>Borrow ID:</strong> ${sanitizeHTML(borrow.id)}<br>
          <strong>Borrower:</strong> ${sanitizeHTML(borrow.borrower)}<br>
          <strong>Fee:</strong> ${(Number(borrow.fee) / 1e18).toFixed(4)} WEEDL<br>
          <strong>Duration:</strong> ${(Number(borrow.duration) / 86400).toFixed(0)} days<br>
          <strong>StartTime:</strong> ${new Date(Number(borrow.startTime) * 1000).toLocaleString()}<br>
          <strong>Location:</strong> (${sanitizeHTML(borrow.latitude || "N/A")}, ${sanitizeHTML(borrow.longitude || "N/A")})<br>
          <strong>Purpose:</strong> ${sanitizeHTML(borrow.purpose || "N/A")}<br>
          <button class="google-maps-btn" data-lat="${sanitizeHTML(borrow.latitude || "0")}" data-lon="${sanitizeHTML(borrow.longitude || "0")}">View on Google Maps</button>
        `,
        (borrow) => ({
          lat: parseFloat(borrow.latitude || "0"),
          lon: parseFloat(borrow.longitude || "0"),
        })
      );
    ...
    const addPlaceholderPlots = () => {
      if (enrichedListings.length || enrichedSeeds.length || enrichedBorrows.length || enrichedLoans.length || (stakeData && stakeData.length)) return;
    ...
    Promise.all([
      ...
      .catch((error) => {
        setError("Failed to add 3D plots.");
  ...
  }, [enrichedListings, enrichedSeeds, enrichedBorrows, enrichedLoans, stakeData, loanData, borrowData, address]);
  ...
  return (
    <div style={{ position: "relative" }}>
      ...
      <canvas
        ...
        role="img"
        aria-label="3D Globe Visualization of Land, Seed, Borrow, Loan, and Stake Listings"
      />
      <div
        ref={tooltipRef}
        style={{
